...
function playerSelec(){
    ...
    fetch(`https://www.thesportsdb.com/api/v1/json/1/lookupplayer.php?id=${idJogador}`).then(res => res.json()).then(data =>{
        ...
        var chuteira = document.createElement('iframe')
        ...
        chuteira.setAttribute('src', `https://www.zoom.com.br/search?q=${item.strKit}`);
        ...
        div.append(nome,fotoJogador,img1,img2,linkInsta,linkFace,linkTwitter,altura,peso,posicao,dscJogador,local,chuteira,data,nacionalidade,time);
        document.getElementById('container').append(div)
        }
    })
}   
